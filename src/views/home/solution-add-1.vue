      ...
      <div style="border: 1px solid #cdcdcd;background-color: #FFFFFF;">
        ...
        <div class="add-table" style="margin-top: 20px;padding: 20px 30px 0;">
          ...
          <el-table :data="tableData.filter(f => f.checked != 1)" style="width: 100%;">
            ...
            <el-table-column prop="type" label="产品系列" width="120"></el-table-column>
...
// 添加ups之后删除添加的
          ...
          if(that.componentsFlag && that.componentsFlag1){
            // 添加套件****
            var a = that.checkableData1();
            if(a == 1){
              // 更新套件的数量
                // alert('a=1')
                that.tableData1.forEach(item => {
                  if (item.isComponent == 1) {
                    item.productNum = Number(item.productNum)+Number(that.sendData.weaverNum) - 1
                  }
                })
            }else{
                // alert('a!=1')
                for (let i = 0; i < that.components.length; i++) {
                  let obj1 = {
                    productId: that.components[i]['productId'],
                    productModel: that.components[i]['productModel'],
                    description: that.components[i]['description'],
                    type: that.components[i]['type'],
                    productNum: weaverNumSed,
                    sku: that.components[i]['sku'],
                    isComponent:1
                  }
                  that.tableData1[that.tableData1.length] = obj1;
                }
            }
            that.componentsFlag1 = false;
          }
          that.total = that.tableData1.length
        })
      },
      ...
      handleChange(value,oldValue, index) {
        ...
        console.log(Number(value) - Number(oldValue))
        if(index.isComponent == 1){
          console.log('is')
          that.tableData1.forEach(item => {
            // 0是产品，1是套件
            if (item.isComponent == 1) {
          ...
          let productNum = Number(value) - Number(oldValue), componentNum =  Number(value) - Number(oldValue);
          //let productNum = value, componentNum =  value;
          if(index.isComponent == 0){
              componentNum = '';
          }else{
              productNum = ''
          }
          if(Number(value) - Number(oldValue) == 0){
            console.log('return')
              return;
          }
        let sndDesignData = {
