        ...
        <div class="add-table" style="padding: 20px 30px;">
          ...
          <el-table :data="tableData1.filter(f => true)" style="width: 100%;">
            <el-table-column prop="" label="" width="90">
              <template slot-scope="scope">
                <el-popconfirm  title="确认删除？" icon="el-icon-info" iconColor="red" @onConfirm="checkhandledel(scope.row)" style="width: 1em;height: 1em;">
            ...
            <el-table-column  prop="interior"  label="数量" width="120">
                <template slot-scope="scope">
                  <el-input-number style="width: 7em;" v-model="scope.row.productNum" @change="((val)=>{handleChange(val, scope.row)})" size="mini" :min="1"></el-input-number>
                </template>
            </el-table-column>
          </el-table>
          <!-- <table>
...
<script>
  import {getProductList,sndDesign} from '@/api/movie'
  export default {
    name: 'solution-add',
    data() {
      return {
    ...
    methods: {
      getProductList(param){
        ...
        getProductList(that.sendData).then((response) => {
          ...
          if(!that.componentsFlag){
            that.components = data.components
            that.componentsFlag = !that.componentsFlag;
          }
          that.flag = 1;
      ...
      checkhandle(row){
        ...
        if(that.sendData.region){
          alert(that.sendData.region)
          alert(that.sendData.weaverNum)
          ...
          if(!that.componentsFlag){
            for(let i = 0; i < that.components.length;i++){
              let obj1 = {
                  ...
                  productModel:that.components[i].component['productId'],
                  description:that.components[i].component['productId'],
                  productNum:that.components[i].component['productId'],
                  sku:that.components[i].component['productId'],
                  productNum:that.components[i].productNum
              }
              console.log(obj1);
